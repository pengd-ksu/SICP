...
;; (stream-head->list (pairs integers integers) 20)

;; The following explanation is from:
;; https://eli.thegreenplace.net/2007/11/10/sicp-section-353, and
;; https://github.com/hjcapple/reading-sicp/blob/master/chapter_3/exercise_3_68.md

;; Calling Louis’s pairs will cause an infinite loop, because there’s no delayed
;; definition like we’re used to have with streams. interleave evaluates its 
;; second argument, which is a recursive call to pairs, which in turn calls itself
;; again and again. In the original version, interleave is called through 
;; cons-stream that doesn’t actually evaluate it until asked for the next element.