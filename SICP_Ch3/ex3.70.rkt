...
;; Exercise 3.70
;; It seems the authors admitted s1 and s2 were already increasing streams as default
(define (merge-weighted s1 s2 weight)
  (cond ((stream-null? s1) s2)
        ((stream-null? s2) s1)
        (else
         (let ((s1w (weight (stream-car s1)))
               (s2w (weight (stream-car s2)))
               (s1car (stream-car s1))
               (s2car (stream-car s2)))
           (cond ((< s1w s2w)
...
;; a
(define sum-positive-pairs
  (weighted-pairs
   integers
   integers
   (lambda (pairs) (apply + pairs))))

;(stream-head->list sum-positive-pairs 10)

;; b
(define non-divide-235
